...
single_phon_freq = 15.348387e12 #Hz
mass_si = 4.65e-26 #~ mass of silicon in kg
Temp = 300 #~ Kelvin
#####################################
nat = 2 #number of total atoms
numsi = 2 #number of silicom atoms
#carbons need to come right after hydrogens or vice versa #THEY CANNOT MIX OF CODE WILL NOT WORK

filename = '511.948002_cm-1_posvec+eigvec'
...
for k in range(nat):
    ...
    if k in range(numsi):
        for j in range(3):
            mod_eig = tau_displacement(mass_si, single_phon_freq, Temp) * unmod_eig[j]
...
#making atom object
phon_atoms = Atoms('Si2')
#cell is manually put in for now, but we should grab it from the CONTCAR
cell_array = [[-2.69880378,    0.0000000000000000,    2.69880378],[0,   2.69880378,   2.69880378],[-2.69880378,   2.69880378,  0]]
