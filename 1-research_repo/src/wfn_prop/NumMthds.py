...
@dataclass
class RungeKutta4_compression:
    # User Inputs
    spatial_solver: object  # UpwindDifference2d or 3d instance
    total_sim_time: float
    courant_number: float = 0.5  # Default Safety factor for stability
    scattering_solver: object = None  # scattering objects in k_scat.py
    ...
    def solve(self, n_initial: np.ndarray, save_interval: int = 10):
        """
        Executes the explicit RK4 time integration with mid-flight compression.
        Returns two separated arrays for real space and momentum space.
        """
        n_current = np.copy(n_initial)

        # Initialize separate compressed tracking lists
        history_real = [np.sum(n_current, axis=2).astype(np.float32)]
        history_mom = [np.sum(n_current, axis=(0, 1)).astype(np.float32)]
        ...
        for step in range(self.num_steps):
            ...
            # Save the compressed frames
            if (step + 1) % save_interval == 0:
                history_real.append(np.sum(n_current, axis=2).astype(np.float32))
                history_mom.append(np.sum(n_current, axis=(0, 1)).astype(np.float32))

            if (step + 1) % 500 == 0:
                print(f"Progress: Step {step + 1} / {self.num_steps} complete.")

        return history_real, history_mom
