...
def Coordinate_convert(b, coord, Cart_true): #NEW CODE
    ...
    Y = coord #gotten form eigvec_list in parseModes
    ...
    if Cart_true == "False": 
        X = np.matmul(M_inv, Y.T).T #give us fractional coordinates [a,b,c]
        print("fractional coodinates: ")
        print(X)
    return X

def calc_freq_displacement_for_one_lattice(b, EV_array, Cart_True): #NEW CODE
...
def parse_poscar(poscar):
    ...
    poscar.seek(0) # just in case #changes file position to 0 (basically want to make sure it starts from the beginning of the POSCAR file
    lines = poscar.readlines()

    scale = float(lines[1])  #grabs scale from poscar (2nd line)
    if scale < 0.0:
        print("[parse_poscar]: ERROR negative scale not implemented.")
        sys.exit(1)

    b = []
    for i in range(2, 5): #grabs lines 3,4, and 5 (cartesian coordinates for the lattice)
        b.append([float(x)*scale for x in lines[i].split()[:3]]) #b is a 3x3 matrix (from poscar)
            ...
            for i in range(nat*3):
                ...
                for j in range(nat):
                    ...
                    if j < natC: 
                        ...
                        EVECx +=[float(tmp[3])] # Here we are adding the eigenvectors together
                        EVECy +=[float(tmp[4])]
                        EVECz +=[float(tmp[5])]
